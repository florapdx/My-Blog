...
function addUpcomingMeetups() {
   $.ajax( {
       url: 'http://api.meetup.com/2/events.json?key=70704f53354b686770246f68494e2637&group_id=4808882&fields=group_photo&time=0m,1m&status=upcoming&sign=true',
       dataType: 'jsonp',
...
function buildHtml(data) {
    ...
    for (var key in data.results) {
      ...
      $('#meetup').find('ul').append('<li>'+html+'</li>');
    }
}
...
function handleError(data) {
    $('#meetup ul').remove();
    $('#meetup').append('<ul></ul>');
    $('#meetup').find('ul').append('<li>Sorry, we are unable to reach Meetup.com at this time</li>');
}
